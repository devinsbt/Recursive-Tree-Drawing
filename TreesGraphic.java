...
public class TreesGraphic {
    public static void main(String[] args) {
        ...
        drawTreesRand(START_X, START_Y, 15);
    ...
    public static void drawTreesRand(double x, double y, double lAngle, double rAngle, double length, double size,
            int n) {
        ...
        if (n >= 0) {
            ...
            drawTreesRand(leftX, leftY, lAngle + ((ANGLE) * Math.random() + ANGLE),
                    lAngle - ((ANGLE) * Math.random() + ANGLE), length - (length / ((max - min) * Math.random() + min)),
                    size, n - 1);

            drawTreesRand(rightX, rightY, rAngle - ((ANGLE) * Math.random() + ANGLE),
                    rAngle + ((ANGLE) * Math.random() + ANGLE), length - (length / ((max - min) * Math.random() + min)),
                    size, n - 1);
        }
    }
